...
# read content
with open(os.path.join(os.path.dirname(__file__), '../data/content.txt'), 'r') as p, open(os.path.join(os.path.dirname(__file__), '../data/content.html'), 'r') as h:
    plain = p.read()
    html = h.read()
...
def notify(email: str, number: int, win):
    ...
    # fill placeholders
    subject = subject_win if win else subject_lose
